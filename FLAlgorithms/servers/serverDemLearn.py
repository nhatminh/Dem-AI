...
class DemLearn(Dem_Server):
    def __init__(self, experiment, device, dataset, algorithm, model, batch_size, learning_rate, beta, L_k,
        ...
        super().__init__(experiment, device, dataset, algorithm, model[0], batch_size, learning_rate, beta, L_k,
                         num_glob_iters,
        ...
        for i in range(self.total_users):
            ...
            print("User ", id, ": Numb of Training data", len(train), ", Testing data", len(test))
            if (self.sub_data):
                print(">>> Cutoff >>>>")
                if (i in randomList):
                    train, test = self.get_data(train, test)

            user = UserDemLearn(device, id, train, test, model, batch_size, learning_rate, beta, L_k, local_epochs,
                                optimizer, K, personal_learning_rate, args)
    ...
    def train(self):
        ...
        for i in range(self.num_glob_iters):
            ...
            # mu_t = max(mu_t*0.9,0.001) #### Mnist dataset
            # mu_t = min(mu_t * 1.1, 3)  #### Mnist dataset
